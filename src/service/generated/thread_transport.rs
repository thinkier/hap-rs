...
use crate::{
    characteristic::{
        ...
        thread_openthread_version::ThreadOpenthreadVersionCharacteristic, thread_status::ThreadStatusCharacteristic,
        transmit_power::TransmitPowerCharacteristic, HapCharacteristic,
    },
    service::HapService,
    HapType,
};
...
impl ThreadTransportService {
    ...
    pub fn new(id: u64, accessory_id: u64) -> Self {
        Self {
            ...
            cca_energy_detect_threshold: Some(CcaEnergyDetectThresholdCharacteristic::new(
                id + 1 + 0 + 4,
                accessory_id,
            )),
            cca_signal_detect_threshold: Some(CcaSignalDetectThresholdCharacteristic::new(
                id + 1 + 1 + 4,
                accessory_id,
            )),
            event_retransmission_maximum: Some(EventRetransmissionMaximumCharacteristic::new(
                id + 1 + 2 + 4,
                accessory_id,
            )),
            event_transmission_counters: Some(EventTransmissionCountersCharacteristic::new(
                id + 1 + 3 + 4,
                accessory_id,
            )),
            mac_retransmission_maximum: Some(MacRetransmissionMaximumCharacteristic::new(
                id + 1 + 4 + 4,
                accessory_id,
            )),
            ...
            received_signal_strength_indication: Some(ReceivedSignalStrengthIndicationCharacteristic::new(
                id + 1 + 7 + 4,
                accessory_id,
            )),
